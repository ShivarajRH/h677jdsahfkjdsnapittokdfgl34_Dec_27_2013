...
//            print_r($data);die();
	...
	function cancel_invoice($invno="")
	{
		...
		foreach($orders as $o)
		{
			...
			$this->db->query("update t_imei_no set status=0 where order_id=? and status = 1 ",$o['id']);
		}
...
// 		$this->form_validation->set_rules('pissno', 'Is Active', 'required');
	...
	function jx_pnh_getmid()
	{
		$user=$this->auth();
		$mid=$this->input->post('mid');
		$mem_mob=$this->input->post('member_mobno');
		if($mid)
		
			$mem=$this->db->query("select * from pnh_member_info where pnh_member_id=?",$mid)->row_array();
		else
			$mem=$this->db->query("select * from pnh_member_info where mobile=?",$mem_mob)->row_array();
	
		if($mid && $mem_mob)
			$mem=$this->db->query("select * from pnh_member_info where pnh_member_id=? and mobile=?",array($mid,$mem_mob))->row_array();
		
		if(empty($mem))
		{
			if($mid)
				die("$mid is not registered");
			if($mem_mob)
				die("$mem_mob is not registered");
			if($mid && $mem_mob)
				die("$mid,$mem_mob is not matched");
		}
		else 
		{
...
//----------------------------------------------------END OF TASK PRINT----------------------------------------------------------------------     
	 ...
	 *
	 */
	function pnh_invoice_returns($date_from="0000-00-00",$date_to="0000-00-00",$srch=0,$franchise='0',$stat=0,$pg=0)
	{
		...
		$this->load->library('pagination');
		$config['base_url'] = site_url('/admin/pnh_invoice_returns/'.$date_from.'/'.$date_to.'/'.$srch.'/'.$franchise.'/'.$stat);
...
die; */
	...
	function jx_getimeidet()
	{
		...
		{	
			...
			{
				// get imei details 
				$sql = "select b.userid,f.invoice_no,b.transid,c.franchise_id,franchise_name,e.name,a.imei_no,b.status,imei_scheme_id,imei_reimbursement_value_perunit,member_id,from_unixtime(c.init) as ordered_on,a.is_imei_activated
								from t_imei_no a 
								join king_orders b on a.order_id = b.id  
								join king_transactions c on b.transid = c.transid 
								join pnh_m_franchise_info d on d.franchise_id = c.franchise_id 
