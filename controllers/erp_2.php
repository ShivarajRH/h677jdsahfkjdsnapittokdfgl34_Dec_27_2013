...
class Erp extends Controller
{
	...
	function __construct()
	{
		...
		if($user_session)
		{
			$user_access_list_res = $this->db->query("select module_name,const_name as access from m_erp_role_access_link a join user_access_roles1 b on b.id = a.access_id where role_id = ? and is_active = 1",$user_session['role_id']);
			if($user_access_list_res->num_rows())
			{
				foreach($user_access_list_res->result_array() as $user_access)
				{
					if(!isset($user_access_list[$user_access['module_name']]))
						$user_access_list[$user_access['module_name']] = array();
						
					if(!isset($user_access_list[$user_access['module_name']][$user_access['access']]))
						$user_access_list[$user_access['module_name']][$user_access['access']] = 1;
						 
				}
			}
		}
		$this->user_access = $user_access_list;
	...
	function check_featureauth($mod,$access,$return=false)
	{
		$user_session = $this->session->userdata('admin_user');
		...
		$allow = false;
		if(isset($this->user_access[$mod]))
			if(isset($this->user_access[$mod][$access]))
				$allow = true;
				
		if(!$allow)
			return !$return?show_error("You are not authorised for this process/action "):false;
		else
			return $user_session;
	...
	/**
	 * function to List Available Storage Rackbins  
	 */
	function rackbins()
	{
		$user = $this->check_featureauth('storage_rackbin','list');
	...
	/**
	 * function to create new Rackbin 
	 */
	function addrackbin()
	{
		$user = $this->check_featureauth('storage_rackbin','add');
	...
	{
		$user = $this->check_featureauth('product','list');
	...
	/**
	 * function to view linked deals for product  
	 */
	function viewlinkeddeals($pid)
	{
		$user = $this->check_featureauth('product','view');
	...
	/**
	 * function to update product barcode in master 
	 */
	...
	/**
	 * function to update product sourceable status 
	 */
	function mark_src_products()
	{
		$user = $this->check_featureauth('product','add|update_sourceable_status');
	...
	/**
	 * function to update Cheque Status as Submitted 
	 */
	function update_check_issubmitted()
	{
		$user = $this->check_featureauth('receipts','add|edit|submit_cheque_to_bank');
	...
	/**
	 * function to Create Product Group 
	 */
	function add_products_group()
	{
		$user = $this->check_featureauth('product','add');
		if($_POST)
	...
	/**
	 * function to Create Product Group  Category 
	 */
	...
	/**
	 * function to list product groups 
	 */
	...
	/**
	 * function to view product group 
	 */
	...
	/**
	 * function to create bulk product groups via CSV file  
	 */
	...
	/**
	 * function to create bulk deals via CSV file  
	 */
	function pnh_deals_bulk_upload()
	{
		$user = $this->check_featureauth('deal','add');
		if($_FILES)
	...
	{
		$user = $this->check_featureauth('deal','auto_image_updater');
		if($_POST)
		{
	...
	{
		$user = $this->check_featureauth('variant','view');
	...
	function variants()
	{
		$user = $this->check_featureauth('variant','list');
	...
	function addvariant()
	{
		$user = $this->check_featureauth('variant','add');
		if($_POST)
	...
	function vendor($vid=false)
	{
		...
		$user = $this->check_featureauth('vendor','list');
	...
	function editvendor($vid=false)
	{
		...
		$user = $this->check_featureauth('vendor','edit');
	...
	function addvendor()
	{
		$user = $this->check_featureauth('vendor','add');
	...
	{
		$user = $this->check_featureauth('purchaseorder','edit|close_purchase_order');
	...
	{
		$user = $this->check_featureauth('ticket','edit|assign_admin_to_ticket');
...
//			preg_match("/SNP[[:alpha:]][[:alpha:]][[:alpha:]][[:digit:]][[:digit:]][[:digit:]][[:digit:]][[:digit:]]/i",$m['subject'],$matches);
	...
	{
		$user = $this->check_featureauth('accounts','view_voucher');
	...
	{
		$user = $this->check_featureauth('accounts','show_vouchers');
	...
	function create_voucher_exp()
	{
		...
		$user = $this->check_featureauth('accounts','create_voucher');
	...
	{
		$user = $this->check_featureauth('ticket','edit|assign_admin_to_ticket',true);
	...
	{
		$user = $this->check_featureauth('ticket','view',true);
	 	echo $this->db->query("select m.msg from support_tickets_msg m where m.id=?",$id)->row()->msg;
	}
	
	function ticket($tid)
	{
		$user = $this->check_featureauth('ticket','edit|update');
		if($_POST)
		{
			$action=$this->input->post("action");
			if($action==0)
	...
	function addticket()
	{
		$user = $this->check_featureauth('ticket','add');
		if($_POST)
			$this->erpm->do_ticket();
		$data['page']="addticket";
		$this->load->view("admin",$data);
	...
	{
		$user = $this->check_featureauth('ticket','list');
		if($s!=false && $e!=false && (strtotime($s)<=0 || strtotime($e)<=0))
			show_404();
	...
	{
		$user = $this->check_featureauth('accounts','account_grn');
	...
	{
		$user = $this->check_featureauth('purchaseorder','view');
	...
	{
		$user = $this->check_featureauth('accounts','pending_grn_payments');
	...
	{
		$user = $this->check_featureauth('grn','list');
	...
	{
		$user = $this->check_featureauth('purchaseorder','list');
	...
	{
		$user = $this->check_featureauth('storage_location','add');
	...
	{
		$user = $this->check_featureauth('storage_location','list');
	...
	{
		$user = $this->check_featureauth('grn','view');
	...
	{
		$user = $this->check_featureauth('product','view_price_changelog');
	...
	{
		$user = $this->check_featureauth('product','view_sourceable_changelog');
		if(empty($e))
		{
			$s=time()-(30*24*60*60);
			$e=time();
	...
	{
		$user = $this->check_featureauth('deal','view_deals_by_not_sourceable_product_log');
	...
	{
		$user = $this->check_featureauth('franchise','update_app_version');
		if(!$v)
		{
	...
	{
		$user = $this->check_featureauth('deal','view_deal_price_changelog');
...
//		else
	...
	{
		$poid="";
		$user = $this->check_featureauth('grn','add');
		if($_POST)
		{
	...
	function po_product()
	{
		$user = $this->check_featureauth('purchaseorder','add');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('product','download_stock_unavail_report');
	...
	{
		$user = $this->check_featureauth('product','view_stock_unavail_report_for_pnh_orders');
		if($_POST)
		{
	...
	function partial_shipment($s='',$e='')
	{
		...
		$user = $this->check_featureauth('order','list');
	...
	{
		$user = $this->check_featureauth('order','update');
		if(empty($transid))
	...
	{
		$user = $this->check_featureauth('order','update|set_priority');
	...
	{
		$user = $this->check_featureauth('order','cancel_order');
		$r=0;
		if(empty($_POST))
	...
	{
		$user = $this->check_featureauth('stock','stock_correction');
	...
	{
		$user = $this->check_featureauth('order','cancel_invoice');
...
//		if($this->db->query("select count(1) as l from shipment_batch_process_invoice_link bi join proforma_invoices i on i.p_invoice_no=bi.p_invoice_no where bi.batch_id=? and bi.packed=0 and i.invoice_status=1",$bid)->row()->l==0)
	...
	{
		$user = $this->check_featureauth('order','process_refund');
	...
	{
		$user = $this->check_featureauth('order','update|change_order_qty');
	...
	{
		$user = $this->check_featureauth('order','enable_disable_for_batch');
		$flag=1;
	...
	{
		$user = $this->check_featureauth('order','resend_shipment_mail');
	...
	{
		$user = $this->check_featureauth('order','resend_order_confirmation_mail');
	...
	{
		$user = $this->check_featureauth('order','reship_order');
		if(empty($_POST))
	...
	function trans($transid="")
	{
		if(empty($transid))
			show_404();
		$user = $this->check_featureauth('order','view');
	...
	{
		$user = $this->check_featureauth('accounts','view_pending_refunds');
	...
	{
		$user = $this->check_featureauth('reports','investor_report');
	...
	{
		$user = $this->check_featureauth('user','change_password');
	...
	{
		$user = $this->check_featureauth('batch','create_batch');
		if(!$_POST)
	...
	{
		$user = $this->check_featureauth('courier','add');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('courier','edit');
		if(!$cid)
			show_404();
		if($_POST)
		{
	...
	function courier()
	{
		$user = $this->check_featureauth('courier','list');
	...
	{
		$user = $this->check_featureauth('courier','update_awb_nos');
		if($_POST)
		{
	...
	{
		$user = $this->check_featureauth('order','update_shipment_kfile');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('partner','add|edit');
		if($pid)
...
//			$this->erpm->debug_post();
	...
	function partners()
	{
		$user = $this->check_featureauth('partner','list');
	...
	{
		$user = $this->check_featureauth('partner','manage_partner_orders');
	...
	function warehouse_summary($type=0,$id=0)
	{
		...
		$user = $this->check_featureauth('stock','view');
	...
	{
		$user = $this->check_featureauth('shipment','generate_kfile');
	...
	function generate_manifesto($pg=0,$pg2=0)
	{
		...
		$user = $this->check_featureauth('shipment','generate_manifesto');
	...
	{
		$user = $this->check_featureauth('shipment','view_pending_shipments');
	...
	{
		$user = $this->check_featureauth('manifesto','add|edit|view|list');
	...
	{
		$user = $this->check_featureauth('manifesto','print');
		$user = $this->auth();
	...
	function packed_list($s=0,$e=0)
	{
		...
		$user = $this->check_featureauth('invoice','view_packed_list');
	...
	{
		$user = $this->check_featureauth('invoice','view_outscan_list');
		if($s==0)
		{
	...
	{
		if($is_pnh)
			$user = $this->check_featureauth('invoice','invoice_packedscan');
		else 
			$user = $this->check_featureauth('invoice','invoice_shippedscan');
	...
	{
		$user = $this->check_featureauth('invoice','cancel_proforma_invoice');
		...
		$is_batch_open = $this->db->query("select count(*) as t from (
			select  a.batch_id,a.p_invoice_no,sum(1) as total,sum(b.invoice_status) as ttl_active,sum(if(a.invoice_no,1,0)) as ttl_invoiced 
				...
				where c.status = 1 and a.batch_id = ?  
			group by a.batch_id
	...
	{
		$user = $this->check_featureauth('invoice','view_proforma_invoice');
	...
	{
		$user = $this->check_featureauth('invoice','pack_invoice_orders');
	...
	function batch($bid)
	{
		...
		$user = $this->check_featureauth('batch','view');
	...
	{
		$user = $this->check_featureauth('batch','view_stock_procure_list');
	...
	{
		$user = $this->check_featureauth('batch','view_product_procure_list');
	...
	{
		$user = $this->check_featureauth('batch','add');
	...
	{
		$user = $this->check_featureauth('batch','view_pending_batch');
	...
	{
		$user = $this->check_featureauth('receipt','reverse_receipt');
	...
	{
		$user = $this->check_featureauth('receipt','update_cheque_transit_status');
	...
	function pnh_acc_stat_c($fid)
	{
		...
		$user = $this->check_featureauth('accounts','account_statement_correction');
	...
	{
		$user = $this->check_featureauth('product','edit');
		if($_POST){
	...
	{
		$user = $this->check_featureauth('adminuser','reset_password');
	...
	{
		$user = $this->check_featureauth('adminuser','edit');
	...
	{
		$user = $this->check_featureauth('adminuser','add');
		if($_POST)
	...
	function adminusers()
	{
		$user = $this->check_featureauth('adminuser','list');
	...
	{
		// depr
		//$user = $this->check_featureauth('roles','manage_roles');
	...
	function addbrand()
	{
		$user = $this->check_featureauth('brand','add');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('brand','view');
		$data['page']="viewbrand";
	...
	{
		$user = $this->check_featureauth('brand','edit');
		if(empty($bid))
			show_404();
		if($_POST)
	...
	{
		$user = $this->check_featureauth('category','view');
		if(empty($cat))
			show_404();
	...
	function addcat()
	{
		$user = $this->check_featureauth('category','add');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('category','edit');
		if(empty($catid))
			show_404();
		if($_POST)
	...
	{
		exit;
		$user = $this->check_featureauth('reports','download_masters');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('deal','mark_publish_and_unpublish');
		if(!$_POST)
			die;
	...
	{
		$user = $this->check_featureauth('deal','mark_live_and_notlive');
		if(!$_POST)
			die;
	...
	{
		$user = $this->check_featureauth('deal','bulk_image_update');
	...
	{
		$user = $this->check_featureauth('deal','list');
	...
	function categories()
	{
		$user = $this->check_featureauth('category','list');
	...
	function brands()
	{
		$user = $this->check_featureauth('brand','list');
	...
	{
		$user = $this->check_featureauth('client','view_invoices');
		if($e)
			$data['pagetitle']="between $s and $e";
	...
	{
		$user = $this->check_featureauth('client','print_invoices');
	...
	{
		$user = $this->check_featureauth('client','pack_invoice');
		if($_POST)
		{
	...
	function payment_client_invoice($inv=false)
	{
		if(!$inv)
			show_404();
		$user = $this->check_featureauth('accounts','client_invoice_payment');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('client','view_invoice');
		if(!$iid)
			show_404();
	...
	{
		$user = $this->check_featureauth('client','create_invoice');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('client','view_order');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('client','list_orders');
	...
	function clients()
	{
		$user = $this->check_featureauth('client','list');
	...
	{
		$user = $this->check_featureauth('client','add_order');
	...
	{
		$user = $this->check_featureauth('client','edit');
		if($_POST)
	...
	function addclient()
	{
		$user = $this->check_featureauth('client','add');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('stock','stock_checker');
		if($_POST)
		{
	...
	{
		$user = $this->check_featureauth('partner','update_partner_deal_prices');
		if($_POST)
		{
	...
	{
		$user = $this->check_featureauth('partner','view_partner_deal_prices|update_partner_deal_prices');
		if($_POST)
		{
	...
	{
		$user = $this->check_featureauth('franchise','add_request');
	...
	{
		$user = $this->check_featureauth('franchise','update_request');
		if($_POST)
		{
	...
	function pnh_quotes($fid=0,$s=0,$e=0,$brand=0,$fra=0,$sby=0,$sord=0,$pg=0)
	{
		...
		$user = $this->check_featureauth('franchise','view_requests');
		if($brand)
		{
	...
	{
		$user = $this->check_featureauth('franchise','view_request');
		if($_POST)
		{
			$id=$_POST['id'];
	...
	{
		$user = $this->check_featureauth('franchise','place_order');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('franchise','update_scheme_discount');
	...
	{
		$user = $this->check_featureauth('franchise','update_superscheme');
	...
	{
		$user = $this->check_featureauth('deal','update_special_margin');
	...
	{
		$user = $this->check_featureauth('deal','disable_special_margin');
		if(!empty($id))
	...
	{
		$user = $this->check_featureauth('deal','edit');
	...
	{
		$user = $this->check_featureauth('deal','add|edit');
	...
	{
		$user = $this->check_featureauth('franchise','edit');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('franchise','assign_executives');
	...
	{
		$user = $this->check_featureauth('franchise','add');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('town','add|edit');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('town','list');
	...
	{
		$user = $this->check_featureauth('franchise','print_franchises');
	...
	{
		$user = $this->check_featureauth('franchise','list');
		$data['frans']=$this->erpm->pnh_getfranchisesbyterry($tid);
		$data['pagetitle']="Franchises of territory:".$this->db->query("select territory_name as n from pnh_m_territory_info where id=?",$tid)->row()->n;
	...
	{
		$user = $this->check_featureauth('receipt','activate_receipt');
		if(!$rid)
	...
	{
		$user = $this->check_featureauth('receipt','cancel_receipt');
		if(!$rid)
			show_error("Input Missing");
		else
	...
	{
		$user = $this->check_featureauth('franchise','update_scheme_status');
	...
	{
		$user = $this->check_featureauth('receipt','view_pending_receipts');
		if($_POST)
		{
			if($_POST['type']=="act")
	...
	{	
		$user = $this->check_featureauth('receipt','list');
	...
	{
		$user = $this->check_featureauth('receipt','view_receipt_update_log');
	...
	{
		$user = $this->check_featureauth('franchise','download_account_statement');
	...
	{
		$user = $this->check_featureauth('log','view_sms_log');
	...
	{
		$user = $this->check_featureauth('miscellaneous','view_company_details');
	...
	{
		$user = $this->check_featureauth('franchise','view');
	...
	{
		$user = $this->check_featureauth('franchise','view_unordered_franchises_log');
	...
	{
		$user = $this->check_featureauth('franchise','manage_unordered_franchises');
	...
	{
		$user = $this->check_featureauth('deal','view_special_margin_deals');
	...
	{
		$user = $this->check_featureauth('franchise','add_credit');
	...
	{
		$user = $this->check_featureauth('franchise','view_franchise_scheme_discounts');
	...
	{
		$user = $this->check_featureauth('franchise','update_bulk_scheme_discounts');
	...
	{
		$user = $this->check_featureauth('franchise','add_scheme_discount');
	...
	{
		$user = $this->check_featureauth('franchise','add_superscheme');
	...
	function pnh_loyalty_points()
	{
		...
		$user = $this->check_featureauth('loyalty_points','manage');
	...
	{
		$user = $this->check_featureauth('member','list');
	...
	{
		$user = $this->check_featureauth('member','view');
	...
	{
		$user = $this->check_featureauth('franchise','allot_memberids');
	...
	{
		$user = $this->check_featureauth('franchise','update_bank_details');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('franchise','add_topup');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('franchise','manage_device');
	...
	{
		$user = $this->check_featureauth('device','manage');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('product','mrp_update');	
	...
	function pnh_class()
	{
		$user = $this->check_featureauth('franchise','manage_class');
	...
	{
		$user = $this->check_featureauth('territory','add|edit|view|list');
	...
	{
		$user = $this->check_featureauth('order','process_bulk_import_orders');
		if($_FILES)
	...
	{
		$user = $this->check_featureauth('deal','process_bulk_upload_images');
		if($_FILES)
	...
	{
		$user = $this->check_featureauth('member','add');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('member','edit');
		if($_POST)
	...
	{
		$user = $this->check_featureauth('order','print_cash_bill');
	...
	{
		$user = $this->check_featureauth('order','create_cash_bill');
	...
	{
		$user = $this->check_featureauth('member','manage_member_cards');
		if($_POST)
		{
			$n=$this->input->post("n");
	...
	{
		$user = $this->check_featureauth('deal','update_images');
		if($_FILES)
	...
	{
		$user = $this->check_featureauth('deal','view');
	...
	{
		$user = $this->check_featureauth('deal','bulk_update_description');
		if($_FILES)
		{
	...
	{
		$user = $this->check_featureauth('campaign','manage_sms_campaign');
		if($_POST)
		{
			foreach(array("fids","send_to","msg","type","sel_itemid","sel_itemname","sel_itemmrp","sel_itemprice","notify_emp","sms_tmpl") as $i)
				$$i=$this->input->post($i);
			if(empty($fids))
	...
	{
		$user = $this->check_featureauth('deal','download_catalog');
	...
	{
		$user = $this->check_featureauth('brand','manage_lessmargin_brands');
		if($_POST)
		{
	...
	{
		$user = $this->check_featureauth('franchise','suspend_or_unsuspend_franchise');
		$fran=$this->db->query("select * from pnh_m_franchise_info where franchise_id=?",$fid)->row_array();
		if(empty($fran))
	...
	{
		$user = $this->check_featureauth('miscellaneous','clear_deals_cache');
	...
	{
		$user = $this->check_featureauth('deal','advanced_deal_report');
		$this->load->library("pettakam",array("repo"=>"cache","ext"=>"pkm_snp"));
	...
	{
		$user = $this->check_featureauth('order','process_shipment_notification_sms');
		if($_POST)
		{
	...
	{
		$user = $this->check_featureauth('adminuser','change_password');
		if($_POST)
		{
			$p=$this->input->post("p");
	...
	{
		$user = $this->check_featureauth('franchise','update_call_response_messages');
	...
	function makeacall()
	{
		$user = $this->check_featureauth('miscellaneous','make_call');
		if($user==false)
			die("0");
	...
	{
		$user = $this->check_featureauth('orders','view_sales_analytics');
	...
	function createpaf()
	{
		$user = $this->check_featureauth('paf','add');
		$data['page']='createpaf';
		$this->load->view("admin",$data);
	}
	...
	{
		$user = $this->check_featureauth('paf','edit');
		if(!$pafid)
	...
	function paflist()
	{
		$user = $this->check_featureauth('paf','list');
	...
	{
		$user = $this->check_featureauth('paf','cancel_paf');
		if(!$_POST)
			die();
		$output = array();
	...
	{
		$user = $this->check_featureauth('paf','process_stockintake_by_paf');
		if(!$paf_id)
			die();
	...
	{
		$user = $this->check_featureauth('paf','print_paf|view_paf');
	...
	{
		$user = $this->check_featureauth('paf','export_products_in_paf');
		if(!$paf_id)
		{
			echo '<script>alert("Invalid paf / products not found in paf")</script>';
			exit;
	...
	function pnh_sales_report()
	{
		...
		$user = $this->check_featureauth('sales','storeking_sales_report');
	...
	{
		$user = $this->check_featureauth('sales','view_employee_sales_summary');
	...
	{
		$user = $this->check_featureauth('sales','export_sales_summary');
	...
	function jx_upd_bulkprodsrcstatus()
	{
	 ...
	 function export_pnh_sales_report()
	 {
	 	...
	 	$user = $this->check_featureauth('sales','download_storeking_sales');
	 	
	 	//ssql to generate franchise list 
	 	$sql_f = "select pnh_franchise_id,franchise_id,franchise_name,b.territory_name,c.town_name  
	 	...
	 	$fr_sales_list[] = '';
	 	$fr_sales_list[] = '';
	 	$fr_sales_list[] = '"Slno","Territory","Town","Status","FranciseID","FranchiseName","Ordered","Shipped","Invoiced Not Shipped","Cancelled","Paid Till Date","UnCleared","Corrections","Pending Payment"," Before 3 Days","Before 5 Days","Before 7 Day","Before 10 Day"';
	 	...
	 	if($res_f->num_rows())
	 	{
	 		...
	 		foreach($res_f->result_array() as $row_f)
	 		{
				...
				$fr_sales_det[] = $shipped_tilldate-$paid_tilldate+$acc_adjustments_val;
				...
				foreach($past_sales_summ as $k)
				{
					...
					$pen_pay_byday = ($shipped_tilldate_byday-$paid_tilldate_byday+$acc_adjustments_val);
	 ...
	 {
	 	$user = $this->check_featureauth('order','bulk_cancel_orders');
	...
	{
		$user = $this->check_featureauth('report','view_unavail_product_ageing_report');
	...
	/**
	 ...
	 {
	 	$user = $this->check_featureauth('shipment','update_partnerorder_manifesto');
	...
	{
		$user = $this->check_featureauth('shipment','download_outscanreport');
...
//-----------------------------------------EMPLOYEE ADD --------------------------------------------------------------------------
	...
	{
		$user = $this->check_featureauth('employee','add');
...
//-----------------------------------------------EDIT EMPLOYEE-----------------------------------------------------------------------
	...
	{
		$user = $this->check_featureauth('employee','edit');
		//$user=$this->auth();
		$user = $this->auth_pnh_employee();
		 if(!$emp_id)
			show_404(); 
	...
	function get_terry_link_byempid($role_id)
		...
		{
			$user = $this->check_featureauth('employee','suspend_and_unsuspend_employee');
...
//--------------------------------------------------VIEW EMPLOYEE--------------------------------------------------------------------------------	
	...
	{
		$user = $this->check_featureauth('employee','view');
		$this->task_status = $this->config->item('task_status');
		$user=$this->auth();
...
//-----------------------------------------------LIST EMPLOYEE-------------------------------------------------------------------	
	...
	{
		$user = $this->check_featureauth('employee','list');
...
//-----------------------Assignment History-------------------------------	
	...
	{
		$user = $this->check_featureauth('employee','view_assignment_histroy');
...
//-------------------------------------------CALANDER-----------------------------------------------------------------------	
	...
	function calender()
	{
		$user = $this->check_featureauth('employee','view_tasks');
		$user = $this->auth_pnh_employee();
	...
	function assigned_location($emp_id)
	{
		...
		function get_franchisebytwn_id($twn_id)
		{
			...
			$franchise_list = $this->db->query("SELECT franchise_id,franchise_name
					...
					WHERE is_suspended=0 and town_id=? ", $twn_id);
			$output=array();
		...
		function get_franchisebyterr_id($terr_id)
		{
			$user=$this->auth_pnh_employee();
			$franchise_list = $this->db->query("SELECT franchise_id,franchise_name
					FROM `pnh_m_franchise_info`
					WHERE is_suspended=0 and territory_id=? ", $terr_id);
			$output=array();
			if($franchise_list ->num_rows())
			{
				$output['franchise_list'] = $franchise_list->result_array();
...
//---------------------------------------------TASK PRINT---------------------------------------------------------------- 
     ...
     {
     	$user = $this->check_featureauth('employee','print_tasks');
...
//----------------------------------------------------END OF TASK PRINT----------------------------------------------------------------------     
	...
	{
		$user = $this->check_featureauth('product_returns','list');
	...
	{
		$user = $this->check_featureauth('product_returns','add');
	...
	{
		$user = $this->check_featureauth('product_returns','view');
	...
	{
		$user = $this->check_featureauth('product_returns','generate_return_receipt');
	...
	{
		$user = $this->check_featureauth('product_returns','update_return_to_packed_status');
	...
	function product_order_summary()
	{
		...
		$user = $this->check_featureauth('product','download_order_summary');
		...
		{
			$user = $this->check_featureauth('miscellaneous','view_storeking_sms_log');
		...
		{
			$user = $this->check_featureauth('deal','deals_bulk_update_by_csv');
			if($_FILES)
			{
		...
		{
			$user = $this->check_featureauth('manifesto','update');
		...
		{
			$user = $this->check_featureauth('franchise','view_unordered_franchises');
		...
		{
			$user = $this->check_featureauth('manifesto','view');
		...
		{
			$user = $this->check_featureauth('deal','export_sales_for_tallyimport');
		...
		{
			$user = $this->check_featureauth('deal','view_deal_product_update_log');
		...
		{
			$user = $this->check_featureauth('shipment','view_shipment_transit_log');
		 ...
		 */
		function get_franchisemenuby_terrid($territory_id)
		{
			$user=$this->auth_pnh_employee();
			$franchise_list = $this->db->query("SELECT a.fid,b.franchise_name,a.menuid,m.name
												...
												WHERE b.is_suspended=0 AND a.status=1 AND b.territory_id=? 
												GROUP BY fid",$territory_id);
			$output=array();
			if($franchise_list ->num_rows())
			{
		 ...
		 */
		function get_franchisebymenu_id($menu_id,$territory_id='')
		{
			$cond='';
			
			if($menu_id)
			$cond.=' and a.menuid='.$menu_id;
		
			if($territory_id )
			$cond.=' and b.territory_id='.$territory_id;
		...
		function get_franchisemenuby_twnid($town_id)
		{
			$user=$this->auth_pnh_employee();
			$franchise_list = $this->db->query("SELECT a.fid,b.franchise_name,a.menuid,m.name
					FROM `pnh_franchise_menu_link`a
					JOIN pnh_m_franchise_info b ON b.franchise_id=a.fid
					JOIN pnh_menu m ON m.id=a.menuid
					WHERE b.is_suspended=0 AND a.status=1 AND b.town_id=?
					GROUP BY fid",$town_id);
			$output=array();
			if($franchise_list ->num_rows())
			{
				$output['townmenu_franlist'] = $franchise_list->result_array();
				$output['status']='success';
			}
			else
			{
		...
		{
			$user = $this->check_featureauth('miscellaneous','add_bank');
		...
		{
			$user = $this->check_featureauth('miscellaneous','view_banks');
		...
		{
			$user = $this->check_featureauth('miscellaneous','edit_bank');
			if(!$bankid)
				show_404();
		...
		{
			$user = $this->check_featureauth('receipt','update_deposited_slips');
		...
		function jx_cancel_processedreceipts()
		{
			...
			$user = $this->check_featureauth('receipt','cancel_processed_receipt');
		...
		{
			$user = $this->check_featureauth('shipment','generate_pick_list_for_pending_shipments');
		...
		function pnh_outscan_pending_shipents($id)
		{
			...
			$user = $this->check_featureauth('shipment','outscan_manifesto');			
	...
	{
		$user = $this->check_featureauth('deal','export_deals_with_notsourceable_products');
	...
	{
		$user = $this->check_featureauth('sales','export_franchise_pending_payment_by_order_report');
	...
	{
		$user = $this->check_featureauth('tray','add|edit|list|view');
	...
	*/
	...
	{
		$user = $this->check_featureauth('transport','add|edit|list|view');
	...
	*/
	...
	function update_lr_number_in_sent_manifesto()
	{
		...
		$user_det = $this->check_featureauth('shipment','update_lr_number');
	...
	{
		$user_det = $this->check_featureauth('deliveryhub','list');
	...
	function pnh_upd_hubinfo()
	{
		...
		$user_det = $this->check_featureauth('deliveryhub','update');
	...
	{
		$user_det = $this->check_featureauth('shipment','print_deliverylabel');
	...
	{
		$user_det = $this->check_featureauth('product','advanced_product_report');
	...
	{
		$user_det = $this->check_featureauth('deal','update_offer_or_dp_price');
	...
	{
		$user_det = $this->check_featureauth('franchise','view_active_superscheme_log');
		...
		if($year && $month)
		{
			...
			$mnth[11]='November';
			$mnth[12]='December';
			$data['pagetitle']="Super scheme details of ".$mnth[$month].'-'.$year;
		}
	...
	function add_inv_to_existing_manifesto()
	{
		...
		$user_det = $this->check_featureauth('manifesto','update_invoices_to_manifesto');
	...
	{
		$user_det = $this->check_featureauth('manifesto','cancel_manifesto');
	...
	{
		$user_det = $this->check_featureauth('deal','update_superscheme_status');
	...
	{
		$user = $this->check_featureauth('invoice','cancel_packed_status');
	...
	{
		$user = $this->check_featureauth('manifesto','update_alternate_mobile_no');
		if(!$_POST)
			die();
	...
	{
		$user = $this->check_featureauth('invoice','scan_invoice_acknowledgement');
	...
	//dynamic menu config
	/**
	 * function for manage the erp user roles
	...
	function manage_user_roles()
	{
		...
		$access_permissions_list=$this->db->query("select * from user_access_roles1 order by module_name;")->result_array();
	
		if($access_permissions_list)
		{
			$module_access_permission_link=array();
			foreach($access_permissions_list as $permission)
			{
				if(!isset($module_access_permission_link[$permission['module_name']]))
					$module_access_permission_link[$permission['module_name']]=array();
	
				$module_access_permission_link[$permission['module_name']][$permission['id']]=$permission['user_role'];
			}
		}
	
		$data['module_access_permission_link']=$module_access_permission_link;
		$data['erp_user_roles']=$this->db->query("select * from m_erp_user_roles;")->result_array();
		$data['page']='manage_user_roles';
		$this->load->view('admin',$data);
	}
	
	/**
	 * save the user role and access permission link configuration.
	 */
	function userrole_access_permission_link_process()
	{
		$user=$this->erpm->auth(true,true);
	
		$user_role=$this->input->post('user_role');
		$access_permissions=$this->input->post('permission_access');
	
		//first make all the configuration inactive for the purpose of configuration edit
		$param1=array();
		$param1['is_active']=0;
		$param1['modified_on']=cur_datetime();
		$param1['modified_by']=$user['userid'];
		...
		$this->db->update('m_erp_role_access_link',$param1);
	
		$access_value=0;
		if($user_role && $access_permissions)
		{
			foreach($access_permissions as $permission )
			{
				$param=array();
				$param['role_id']=$user_role;
				$param['access_id']=$permission ;
				$param['created_by']=$user['userid'];
				$param['created_on']=cur_datetime();
	
				if(!@$this->db->query("select id from m_erp_role_access_link where role_id=? and access_id=?",array($user_role,$permission))->row()->id)
				{
					$this->db->insert('m_erp_role_access_link',$param);
				}else{
					$this->db->query("update m_erp_role_access_link set is_active=1 where role_id=? and access_id=?",array($user_role,$permission));
				}
	
				$access_value_by_access=@$this->db->query("select value from user_access_roles1 where id=?",$permission)->row()->value;
	
				if($access_value_by_access)
					$access_value+=$access_value_by_access;
				else
					$access_value+=0;
			}
	
			$this->db->query("update m_erp_user_roles set access_value=? where role_id=?",array($access_value,$user_role));
		}
	
		$this->session->set_flashdata("erp_pop_info"," Configured with the selected access permissions");
		redirect('/admin/manage_user_roles');
	}
	
	function jx_get_userrole_access_permission_link()
	{
		$user=$this->erpm->auth(true,true);
		...
		$output['access_permission_det']=$this->db->query("select * from m_erp_role_access_link where role_id=? and is_active=1",$role_id)->result_array();
