...
class Voucher extends Controller {
	...
	*/
	...
	function create_rbs()
	{
		$chrs = array('A','B','C','D','E','F','G');
		$chrs_racks = array(9,5,4,4,10,10,4);
		
		foreach($chrs as $k=>$chr)
		{
			$r = $chrs_racks[$k];
			for($i=1;$i<=$r;$i++)
			{
				for($j=1;$j<=7;$j++)
				{
					...
					$inp['created_by'] = 1;
					$this->db->insert("m_rack_bin_info",$inp);
				}
					echo $chr.$i.'-'.$j.'<br>';
			}
		}
	...
	function fix_imeischdisc()
	{
		$sql = "select d.franchise_id,c.menuid,c.catid,c.brandid,d.init as trans_on,date(from_unixtime(d.init)) as ordered_on,a.id as oid,a.transid,a.quantity,(i_orgprice-(i_discount+i_coup_discount)) as amt 
						from king_orders a 
						join king_dealitems b on a.itemid = b.id 
						join king_deals c on c.dealid = b.dealid 
						join king_transactions d on d.transid = a.transid 
						where c.menuid = 112 and c.catid=9 and d.init >= unix_timestamp('2013-09-01') and imei_scheme_id = 0 and a.status != 3  
				order by ordered_on ";
		...
		if($res->num_rows())
		{
			...
			{
				// check if there is scheme for this order 
				$menuid = $row['menuid'];
				$catid = $row['catid'];
				$brandid = $row['brandid'];
				$transon = $row['trans_on'];
				$franchise_id = $row['franchise_id'];
				$order_id = $row['oid'];
				$transid = $row['transid'];
				
				$sql_sch = "select id,scheme_type,credit_value from imei_m_scheme 
									where menuid = ? and categoryid = ? and brandid = ? and franchise_id = ? and is_active = 1 and ? between scheme_from and scheme_to order by id desc limit 1 
								";
				$res_sch = $this->db->query($sql_sch,array($menuid,$catid,$brandid,$franchise_id,$transon));	
					
				if($res_sch->num_rows())
				{
					$scheme_det = $res_sch->row_array();
					// update king_orders table for scheme
					
					$sch_id = $scheme_det['id']; 
					
					if($scheme_det['scheme_type'] == 1)
						$imei_sch_amt = ($row['amt']*$scheme_det['credit_value']/100);
					else 
						$imei_sch_amt = $scheme_det['credit_value'];
							
					$this->db->query('update king_orders set imei_scheme_id = ?,imei_reimbursement_value_perunit=? where id = ? and transid = ? and imei_scheme_id = 0 ',array($sch_id,$imei_sch_amt,$order_id,$transid));
