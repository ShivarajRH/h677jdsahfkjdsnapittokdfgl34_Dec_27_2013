...
*/
	...
	function do_pack()
	{
		...
		foreach($p_invoice_list as $p_invoice)
		{
			...
			if(!isset($pinv_pids[$p_invoice]))
			{
				$pinv_orders=$this->db->query("select a.transid,a.quantity as qty,a.itemid,a.id from king_orders a join proforma_invoices b on a.id = b.order_id and a.transid = b.transid where b.p_invoice_no  = ? ",$p_invoice_no)->result_array();
				
				// revert cancelled orders stock  
				foreach($pinv_orders as $o)
				{
					...
					foreach($pls as $p)
					{
						...
						$this->db->query("update king_orders set status=0 where id = ? and status = 1 ",$o['id']);
						
					}
				}

				$this->db->query("update proforma_invoices set invoice_status=0 where p_invoice_no=?  ",array($p_invoice_no));
				$this->erpm->do_trans_changelog($pinv_orders[0]['transid'],"Proforma Invoice no $p_invoice_no cancelled");
				continue; 
			}
			...
			foreach($order_list as $p_oids)
			{
				...
				$this->db->query("update king_invoice set  ref_dispatch_id=? where invoice_no = ? ",array($last_dispatch_id+1,$invoice_no));
		...
		if($split_inv_grpno)
			redirect("admin/invoice/".($last_dispatch_id+1));
		else
...
*/		
	...
	function getinvoiceforpacking($inv_no)
	{
		...
		*/
		...
		$ret=$this->db->query("select d.pic,d.is_pnh,e.menuid,i.discount,p.product_id,p.mrp,p.barcode,i.transid,i.p_invoice_no,p.product_name,o.i_orgprice as order_mrp,o.quantity*pl.qty as qty,d.name as deal,d.dealid,o.itemid,o.id as order_id,i.p_invoice_no 
	...
	function do_po_prodwise()
	{
		...
		foreach($_POST['vendor'] as $i=>$v)
		{
			...
			if($po['dp_price'])
			{
				...
				if($po['sch_type']==1)
				{
					$price=$po['dp_price']-($po['dp_price']*$po['margin']/100);
					$po['margin']=$_POST['margin'][$i];
				}
				else 
				{ 
					$price=$po['dp_price']-($po['dp_price']-$po['margin']);
					$po['margin']=(1-($price/$po['dp_price']))*100;
				}
			...
			}else
			{
				...
				{
					$price=$po['mrp']-$po['margin'];
					$po['margin']=(1-($price/$po['mrp']))*100;
				}	
	...
	function createpo()
	{
		...
		foreach($_POST['product'] as $i=>$p)
		{
			...
			if($pt['dp_price'][$i] >0)
			{
				...
				if($pt['sch_type'][$i] != 1)
				{
					$price=$pt['dp_price'][$i]-$pt['margin'][$i];
					$mrgn_prc=(1-($price/$pt['dp_price'][$i]))*100;
				}else
					$mrgn_prc=$pt['margin'][$i];
			}else 
			{
				if($pt['sch_type'][$i] != 1)
				{
					$price=$pt['mrp'][$i]-$pt['margin'][$i];
					$mrgn_prc=(1-($price/$pt['mrp'][$i]))*100;
				}else
					$mrgn_prc=$pt['margin'][$i];
			...
			$inp=array($po,$p,$pt['qty'][$i],$pt['mrp'][$i],$pt['dp_price'][$i],$mrgn_prc,$sv,$pt['sch_type'][$i],$pt['price'][$i],$pt["foc$i"],$pt["offer$i"],$pt['note'][$i]);
			
			$this->db->query("insert into t_po_product_link(po_id,product_id,order_qty,mrp,dp_price,margin,scheme_discount_value,scheme_discount_type,purchase_price,is_foc,has_offer,special_note,created_on)
																			values(?,?,?,?,?,?,?,?,?,?,?,?,now())",$inp);
...
order by action_date";
	...
	function sendsms_franchise_shipments($invoices='',$d_total=0)
	{
		...
		if($res_trans->num_rows())
		{
			...
			{
				$sms_msg="Dear ".$fran_det[$fr_id][0].", your ".implode(',',$fr_sms_list)." is shipped successfully, please expect delivery on ".$datetime->format('d/m/Y');
		...
		$sql = "select d.employee_id,e.name,d.territory_id,b.franchise_id,franchise_name,contact_no,group_concat(distinct a.invoice_no order by a.invoice_no) as invoices 
							...
							join king_transactions b on a.transid = b.transid 
							join pnh_m_franchise_info c on c.franchise_id = b.franchise_id 
							join m_town_territory_link d on d.territory_id = c.territory_id and d.town_id = 0 and d.is_active = 1 
							join m_employee_info e on e.employee_id = d.employee_id and job_title = 4 and e.is_suspended = 0  
