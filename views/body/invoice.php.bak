...
$mem_det = array();	
$invoice_credit_note_res = $this->db->query("select group_concat(id) as id,sum(amount) as amount from t_invoice_credit_notes a where invoice_no in (select invoice_no from king_invoice where split_inv_grpno = ? or ref_dispatch_id = ? or invoice_no = ? ) ",array($invoice_no,$invoice_no,$invoice_no));
				...
				<?php
					$dispatch_id = @$this->db->query("
										select dispatch_id 
											...
											where (b.invoice_no = ?  or split_inv_grpno = ? ) 
										group by a.p_invoice_no ",array($invoice_no,$invoice_no))->row()->dispatch_id;
					if($dispatch_id)
					{					
				?>
